...
const ProfileHook = () => {
  ...
  useEffect(() => {
    if (loading === false) {
      ...
      if (res && res.status === 200) {
        notify("Your profile has been successfully updated", "success");
        ...
        notify("Problem in updating process", "warn");
      }
    }
  }, [loading]);
  ...
  const changePassword = async () => {
    if (confirmNewPassword != newPassword) {
      notify("Check your password", "warn");
      return;
    }
    setLoadingPass(true);
    await dispatch(
  ...
  useEffect(() => {
    if (loadingPass === false) {
      ...
      if (resPass && resPass.status === 200) {
        ...
        notify("Problem in process", "warn");
      }
    }
  }, [loadingPass]);
